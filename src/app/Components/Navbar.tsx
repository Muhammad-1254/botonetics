...
const subMenuHandler =(index :number)=>{
  ...
  return (
    <header
      className="z-50 lg:w-full w-full h-16 lg:h-20  bg-blue-500 sticky top-0 
    
    "
    >
      <div className="relative hidden lg:flex items-center justify-between h-20 px-10">
          ...
          <div>
            <ul className="flex flex-col items-start justify-start mt-10 mx-2 gap-y-5 ">
              {navApi.map((item, index) => (
                <li key={item.id}
                className=''>
                  <div className={`${subMenu == index ? " bg-slate-400/50":"bg-slate-300"}  py-1 px-3 rounded-r border-l-4 border-darkPrimary  `}>
                    <Link href={item.link} className="">
                      ...
                      >
                        {item.title}
                        <span className={``} onClick={() =>subMenuHandler(index)}>
                          {
                            subMenu == index ?
                            <AiOutlineUp /> :
                            <AiOutlineDown /> 
                  ...
                  {/* sub container  */}
                  <ul className={`${subMenu == index ? "max-h-full":"max-h-0" }
                    ...
                    {item.dropDown?.map((subItem,index)=>
                      {
                        ...
                        return(
                      <li className={` ${index == 0 ? "pt-5": 'pt-0'}
